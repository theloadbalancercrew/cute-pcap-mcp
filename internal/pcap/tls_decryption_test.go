...
func testTLSKeylogContents() []byte {
	return []byte("CLIENT_RANDOM " + strings.Repeat("a", 64) + " " + strings.Repeat("b", 64) + "\n")
}
...
func TestResolveTLSKeylogStateMachine(t *testing.T) {
	...
	if err := os.WriteFile(realKeylog, testTLSKeylogContents(), 0o600); err != nil {
		t.Fatal(err)
	}
	commentOnlyKeylog := filepath.Join(keylogRoot, "comments.log")
	if err := os.WriteFile(commentOnlyKeylog, []byte("# generated by a browser\n\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	malformedKeylog := filepath.Join(keylogRoot, "not-keys.log")
	if err := os.WriteFile(malformedKeylog, []byte("CLIENT_RANDOM not-hex "+strings.Repeat("c", 64)+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	cases := []struct {
		...
		{
			name:          "keylog_invalid_when_file_has_no_secret_lines",
			path:          commentOnlyKeylog,
			cfg:           cfgWithKeylog,
			wantStatus:    TLSDecryptionStatusKeylogInvalid,
			wantHasDetail: true,
		},
		{
			name:          "keylog_invalid_when_file_has_malformed_lines",
			path:          malformedKeylog,
			cfg:           cfgWithKeylog,
			wantStatus:    TLSDecryptionStatusKeylogInvalid,
			wantHasDetail: true,
		},
...
// TestValidateTLSKeylogFileRecognizesModernLabels keeps keylog shape
// validation broader than only the legacy TLS 1.2 CLIENT_RANDOM form.
// Values are syntactically valid hex but not real secrets.
func TestValidateTLSKeylogFileRecognizesModernLabels(t *testing.T) {
	keylog := filepath.Join(t.TempDir(), "keys.log")
	body := strings.Join([]string{
		"# comments and blanks are ignored",
		"",
		"CLIENT_RANDOM " + strings.Repeat("a", 64) + " " + strings.Repeat("b", 64),
		"CLIENT_HANDSHAKE_TRAFFIC_SECRET " + strings.Repeat("c", 64) + " " + strings.Repeat("d", 96),
		"SERVER_TRAFFIC_SECRET_0 " + strings.Repeat("e", 64) + " " + strings.Repeat("f", 96),
		"RSA " + strings.Repeat("1", 32) + " " + strings.Repeat("2", 96),
		"UNSUPPORTED_LABEL " + strings.Repeat("3", 64) + " " + strings.Repeat("4", 64),
	}, "\n")
	if err := os.WriteFile(keylog, []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}

	summary, err := validateTLSKeylogFile(keylog)
	if err != nil {
		t.Fatal(err)
	}
	if summary.NonCommentLines != 5 {
		t.Fatalf("NonCommentLines = %d, want 5", summary.NonCommentLines)
	}
	if summary.UsableSecretLines != 4 {
		t.Fatalf("UsableSecretLines = %d, want 4", summary.UsableSecretLines)
	}
}

func TestResolveTLSKeylogInvalidDetailDoesNotLeakKeyMaterial(t *testing.T) {
	keylogRoot := t.TempDir()
	secret := strings.Repeat("c", 64)
	keylog := filepath.Join(keylogRoot, "bad.log")
	if err := os.WriteFile(keylog, []byte("CLIENT_RANDOM not-hex "+secret+"\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	resolved, status, detail := resolveTLSKeylog(keylog, cfg)
	if status != TLSDecryptionStatusKeylogInvalid {
		t.Fatalf("status = %q, want %q", status, TLSDecryptionStatusKeylogInvalid)
	}
	if resolved != "" {
		t.Fatalf("invalid keylog returned resolved path %q", resolved)
	}
	if strings.Contains(detail, secret) {
		t.Fatalf("invalid keylog detail leaked key material: %q", detail)
	}
}
...
func TestFinalizeTLSDecryptionStatus(t *testing.T) {
	cases := []struct {
		...
		{
			name:              "keylog_invalid_carries_limitations_no_keylog_path",
			initialStatus:     TLSDecryptionStatusKeylogInvalid,
			wantStatus:        TLSDecryptionStatusKeylogInvalid,
			wantHasLimitation: true,
		},
	}
...
func TestExplainConnectionPassesKeylogToAnalyzePipeline(t *testing.T) {
	...
	if err := os.WriteFile(keylog, testTLSKeylogContents(), 0o600); err != nil {
		t.Fatal(err)
	}
