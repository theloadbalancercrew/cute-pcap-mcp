...
`cute-pcap-mcp` does **not** ship a `testdata/pcaps/` directory of
fixture files. Every test that needs a pcap synthesizes one in-memory
via `syntheticHTTPPcap`, `syntheticSecretsPcap`, or
`libpcapHeaderOnly`. This is deliberate:
